  ...
  <div>
    ...
    <div>
      ...
      >
        ...
        <el-table-column prop="createDate" label="创立日期" align="center" width="200"></el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="创立日期" prop="createDate">
          <el-date-picker
            type="date"
            placeholder="选择日期"
...
export default {
  ...
  methods: {
    // 访问后端获取表格数据
    async getTableData() {
      const { data: res } = await this.$http.get("/department/manageDep", {
        params: this.page,
      });
      //将data数据赋值表格数组
    ...
    async MinusConfirm(row) {
      const { data: res } = await this.$http.delete(
        "/department/manageDep?id=" + row.id
      );
      this.$message.success("删除部门编号为" + row.id + "的数据成功");
      // this.getTableData();
    },
    // 删除取消按钮
    MinusCancel() {
