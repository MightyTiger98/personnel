<template>
  ...
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>员工考评</el-breadcrumb-item>
    </el-breadcrumb>
      ...
      <div>
        <el-row :gutter="10">
          ...
          <el-col :offset="4" :xs="1" :sm="1" :md="1" :lg="1" :xl="1">
            <el-tooltip class="item" effect="dark" content="添加考评" placement="top">
          ...
          <!-- 删除考评 按钮
          <el-col :xs="1" :sm="1" :md="1" :lg="1" :xl="1">
            <el-tooltip class="item" effect="dark" content="删除考评" placement="top">
          ...
          <!-- 修改考评 按钮
          <el-col :xs="1" :sm="1" :md="1" :lg="1" :xl="1">
            <el-tooltip class="item" effect="dark" content="修改考评" placement="top">
          ...
          <!-- 打印考评信息 按钮 -->
          <el-col :xs="1" :sm="1" :md="1" :lg="1" :xl="1">
            <el-tooltip class="item" effect="dark" content="打印考评信息" placement="top">
    ...
    <div>
      ...
      >
        ...
        <el-table-column fixed prop="id" label="考评编号" width="100" align="center"></el-table-column>
        ...
        <el-table-column prop="appDate" label="考评日期" align="center" width="200"></el-table-column>
        <el-table-column prop="appResult" label="考评结果" align="center" width="150"></el-table-column>
        <el-table-column prop="appContent" label="考评内容" align="center" width="150"></el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="120" align="center">
          <template slot-scope="scope">
            <el-popover placement="left" width="400" trigger="click">
              <div class="check">
                <span>
                <span>考评编号</span>{{ scope.row.id }}
              </span>
              <span>
                <span>员工姓名</span>{{ scope.row.name }}
              </span>
              <span>
                <span>员工编号</span>{{ scope.row.eid }}
              </span>
              <span>
                <span>考评日期</span>{{ scope.row.appDate}}
              </span>
              <span>
                <span>考评结果</span>{{ scope.row.appResult }}
              </span>
              <span>
                <span>考评内容</span>{{ scope.row.appContent}}
              </span>
              <span>
                <span>备注</span>{{ scope.row.remark }}
              </span>
              </div>
              <el-button
                slot="reference"
      ...
      >
        ...
        <el-form-item label="考评日期" prop="appDate">
          <el-date-picker
            type="date"
            placeholder="选择日期"
            v-model="addForm.appDate"
            style="width: 100%;"
            value-format="yyyy-MM-dd"
          ></el-date-picker>
        </el-form-item>
        <el-form-item label="考评结果" prop="appResult">
          <el-input v-model="addForm.appResult" ></el-input>
        </el-form-item>
        <el-form-item label="考评内容" prop="appContent">
          <el-input v-model="addForm.appContent" ></el-input>
        </el-form-item>
    ...
    >
      <el-form
        ...
        size="mini"
      >
        <el-form-item label="考评编号" prop="id">
        ...
        <el-form-item label="考评日期" prop="appDate">
          <el-date-picker
            type="date"
            placeholder="选择日期"
            v-model="editForm.appDate"
            style="width: 100%;"
            value-format="yyyy-MM-dd"
          ></el-date-picker>
        </el-form-item>
        <el-form-item label="考评结果" prop="ecResult">
          <el-input v-model="editForm.appResult" ></el-input>
        </el-form-item>
        <el-form-item label="考评内容" prop="ecContent">
          <el-input v-model="editForm.appContent" ></el-input>
        </el-form-item>
        <el-form-item label="备注" prop="remark">
...
export default {
  ...
  methods: {
    // 访问后端获取表格数据
    async getTableData() {
      const { data: res } = await this.$http.get("/personnel/appraise", {
        params: this.page,
      });
      //将data数据赋值表格数组
    ...
    Edit(row) {
      ...
      this.editForm.appDate = row.appDate;
      this.editForm.appResult = row.appResult;
      this.editForm.appContent = row.appContent;
    ...
    async MinusConfirm(row) {
      const { data: res } = await this.$http.delete(
        "/personnel/appraise?id=" + row.id
      );
      this.$message.success("删除考评编号为" + row.id + "的数据成功");
      // this.getTableData();
    },
    // 删除取消按钮
    MinusCancel() {
